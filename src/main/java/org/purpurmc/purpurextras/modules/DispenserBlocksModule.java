 ...
 *
 * **break-blocks** If a tool category is enabled, that tool dispensed from
 * dispenser will destroy the block in front of it. It will only destroy blocks
 * that tool can destroy and it will destroy them like that tool was used on it,
 * so wooden pickaxe will destroy diamond ore, but will not drop any items.
 *
 * **shears-shear-pumpkin** If enabled, when shears are dispensed and there's a
 * pumpkin in front of a dispenser, shears will be used, making carved pumpkin.
 *
 * **interact-with-cauldron** If enabled, will allow dispensers fill and empty
 * cauldrons.
 *
 * **put-discs-in-jukebox** If enabled, dispensers will be able to insert into
 * or swap music discs in jukeboxes.
 */
public class DispenserBlocksModule implements PurpurExtrasModule, Listener {
    ...
    private static final MaterialSetTag CAULDRON_BUCKETS = new MaterialSetTag(
            new NamespacedKey(PurpurExtrasOG.getInstance(), "cauldron_buckets")).add(Material.BUCKET)
            .add(Material.WATER_BUCKET).add(Material.LAVA_BUCKET).add(Material.POWDER_SNOW_BUCKET);
    ...
    public boolean shouldEnable() {
        ...
        return anyTrue(breakBlockAxe, breakBlockHoe, breakBlockPickaxe, breakBlockShovel, breakBlockShears,
                shearPumpkin, activateJukebox, interactWithCauldron);
    ...
    public void onPreDispense(BlockPreDispenseEvent event) {
        ...
        // Shear pumpkin
        if (shearPumpkin && item.getType().equals(Material.SHEARS) && block.getType().equals(Material.PUMPKIN)) {
        ...
        if (activateJukebox && MaterialTags.MUSIC_DISCS.isTagged(item) && block.getType().equals(Material.JUKEBOX)) {
        ...
        if (interactWithCauldron && Tag.CAULDRONS.isTagged(block.getType()) && CAULDRON_BUCKETS.isTagged(item)) {
            ...
            if (block.getType().equals(Material.CAULDRON)) {
                ...
                switch (dispensedItemType) {
                    ...
                    case LAVA_BUCKET -> {
                        ...
                        emptyCauldronHandler(block, lavaCauldron, item, blockDispenser.getLocation(),
                    ...
                    case WATER_BUCKET -> {
                        ...
                        emptyCauldronHandler(block, waterCauldron, item, blockDispenser.getLocation(),
                    ...
                    case POWDER_SNOW_BUCKET -> {
                        ...
                        emptyCauldronHandler(block, powderSnowCauldron, item, blockDispenser.getLocation(),
            ...
            if (!dispensedItemType.equals(Material.BUCKET))
                return;
            switch (block.getType()) {
                ...
                case POWDER_SNOW_CAULDRON -> {
                    ...
                    if (cauldronLevel.getLevel() < 3)
                        return;
                    fullCauldronHandler(block, item, blockDispenser, Material.POWDER_SNOW_BUCKET,
    ...
    private void fullCauldronHandler(Block cauldron, ItemStack items, org.bukkit.block.Dispenser dispenserBlock,
            Material newItem, Sound uniqueSound)
    {
        ...
        dispenserBlock.getWorld().playSound(dispenserBlock.getLocation(), Sound.BLOCK_DISPENSER_DISPENSE,

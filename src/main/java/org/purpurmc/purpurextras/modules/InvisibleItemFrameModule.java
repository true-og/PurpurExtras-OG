...
/**
 * Right click when sneaking on an item frame with item inside of it will make
 * the item frame invisible. Requires purpurextras.invisibleframes permission.
 */
public class InvisibleItemFrameModule implements PurpurExtrasModule, Listener {
    ...
    public boolean shouldEnable() {
        ...
        DefaultPermissions.registerPermission(invisFramePermission,
                "Allows player to shift-right-click an item frame to turn it invisible", PermissionDefault.OP);
    ...
    public void onItemFrameInteract(PlayerInteractEntityEvent event) {
        ...
        if (itemFrame.getItem().getType().equals(Material.AIR))
            return;
        if (!player.hasPermission(invisFramePermission))
            return;

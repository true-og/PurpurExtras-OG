...
/**
 * If enabled, banners will be able to use more than the limit of 6 layers. In
 * default config up to 6 banner layers are allowed.
 */
public class LoomMaxLayersModule implements PurpurExtrasModule, Listener {
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (currentItem != null && MaterialSetTag.BANNERS.isTagged(currentItem.getType())) {
            ...
            if (doesNewLayerExceedMaxLayers(cursorItem))
                return;
    ...
    private boolean isPlaceAction(InventoryAction action) {
        ...
        return action == InventoryAction.PLACE_SOME || action == InventoryAction.PLACE_ALL
    ...
    private void setOldPatternsToBanner(Player player, ItemStack banner) {
        ...
        if (!playerBannerData.containsKey(player))
            return;

...
/**
 * Prevents players from breaking blocks with loot tables that can regenerate
 * loot.
 */
public class LootBlocksProtectionModule implements PurpurExtrasModule, Listener {
    ...
    protected LootBlocksProtectionModule() {
        ...
        try {
            ...
            messageType = MessageType
                    .valueOf(config.getString("settings.protect-blocks-with-loot.message-type", "CHAT").toUpperCase());
    ...
    private void handleLootBlockDestruction(BlockBreakEvent event) {
        ...
        if (event.getPlayer().hasPermission(permission))
            return;
        if (allowBreakingInSneak && event.getPlayer().isSneaking())
            return;

...
public class OpenIronDoorsWithHandModule implements PurpurExtrasModule, Listener {
    ...
    public boolean shouldEnable() {
        ...
        DefaultPermissions.registerPermission(openIronDoorsPermission, "Allows for opening iron doors with a hand",
                PermissionDefault.TRUE);
        DefaultPermissions.registerPermission(openIronTrapdoorsPermission,
                "Allows for opening iron trapdoors with a hand", PermissionDefault.TRUE);
    ...
    public void onIronDoorOpen(PlayerInteractEvent event) {
        ...
        switch (block.getType()) {
            ...
            case IRON_DOOR -> {
                ...
                if (!doors)
                    return;
                if (!event.getPlayer().hasPermission(openIronDoorsPermission))
                    return;
            ...
            case IRON_TRAPDOOR -> {
                ...
                if (!trapdoors)
                    return;
                if (!event.getPlayer().hasPermission(openIronTrapdoorsPermission))
                    return;
                event.setCancelled(true);
                if (open(block, event.getPlayer())) {
            ...
            default -> throw new IllegalArgumentException(
                    "Unexpected value in OpenIronDoorsWithHandModule: " + block.getType());

...
/**
 * If enabled, sends messages in chat containing amount of players required to
 * sleep based on playersSleepingPercentage gamerule.
 */
public class SleepPercentageMessageModule implements PurpurExtrasModule, Listener {
    ...
    protected SleepPercentageMessageModule() {
        ...
        this.playerSleepMessage = config.getString("settings.chat.send-sleep-percentage-message.player-sleeping",
                "<grey><playername> has fallen asleep. <sleeping> out of <needed> required players in <worldname> are sleeping.");
        this.nightSkipMessage = config.getString("settings.chat.send-sleep-percentage-message.skipping-night",
    ...
    public boolean shouldEnable() {
        ...
        DefaultPermissions.registerPermission(sleepMessageBypass,
                "Allows player to not display a message in chat when they sleep", PermissionDefault.OP);
        if ((playerSleepMessage == null || playerSleepMessage.isBlank())
                && (nightSkipMessage == null || nightSkipMessage.isBlank()))
            return false;
        return PurpurExtrasOG.getPurpurConfig().getBoolean("settings.chat.send-sleep-percentage-message.enabled",
                false);
    ...
    public void onPlayerDeepSleep(PlayerDeepSleepEvent event) {
        ...
        if (playerSleepMessage == null || playerSleepMessage.isBlank())
            return;
        if (event.getPlayer().hasPermission(sleepMessageBypass))
            return;
        ...
        world.sendMessage(miniMsg.deserialize(playerSleepMessage, Placeholder.unparsed("playername", playerName),
    ...
    public void nightSkip(TimeSkipEvent event) {
        ...
        if (!event.getSkipReason().equals(TimeSkipEvent.SkipReason.NIGHT_SKIP))
            return;
        if (nightSkipMessage == null || nightSkipMessage.isBlank())
            return;
